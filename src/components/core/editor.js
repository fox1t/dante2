...
class DanteEditor extends React.Component {
  constructor (props) {
    ...
    const { styles, customStyleFn } = createStyles(['font-size', 'color', 'font-family']) //, 'PREFIX', customStyleMap);
    this.styles = styles
  ...
  decodeEditorContent (rawAsJson) {
    const newContent = convertFromRaw(rawAsJson)
    const editorState = EditorState.createWithContent(newContent, this.decorator)
    return editorState
  }
  ...
  handleUndeletables (editorState) {
    ...
    const undeletableTypes = this.widgets.filter(
    ...
    const undeletablesMap = blockMap
      .filter(function (o) {
        return undeletableTypes.indexOf(o.get('type')) > 0
      })

    if (undeletableTypes.length > 0 && undeletablesMap.size === 0) {
      ...
      EditorState.push(editorState, newContentState, 'change-block')
    }
  ...
  setPreContent () {
    ...
    this.save.editorContent = content
    return this.save.editorContent
  }
  ...
  blockRenderer (block) {
    switch (block.getType()) {
      case 'atomic':
        ...
        // const entityType = Entity.get(entity).getType()
  ...
  handleBlockRenderer (block) {
    ...
    const readOnly = this.state.read_only ? false : null
    const editable = readOnly !== null ? readOnly : dataBlock.editable
    return {
  ...
  blockStyleFn (block) {
    const currentBlock = getCurrentBlock(this.state.editorState)
    const isSelected = currentBlock.getKey() === block.getKey() ? 'is-selected' : ''

    if (this.renderableBlocks().includes(block.getType())) {
      return this.styleForBlock(block, currentBlock, isSelected)
    }

    const defaultBlockClass = this.defaultWrappers(block.getType())
    if (defaultBlockClass.length > 0) {
      return `graf ${defaultBlockClass[0]} ${isSelected}`
    } else {
      return `graf ${isSelected}`
    }
  }
  ...
  styleForBlock (block, currentBlock, isSelected) {
    const dataBlock = this.getDataBlock(block)
    ...
    const selectedClass = (dataBlock.selected_class ? dataBlock.selected_class : '')
    const selectedClassOut = isSelected ? selectedClass : ''

    return `${dataBlock.wrapper_class} ${selectedClassOut} ${selectedFn}`
  }
  ...
  handleReturn (e) {
    ...
    if (!e.altKey && !e.metaKey && !e.ctrlKey) {
      ...
      const configBlock = this.getDataBlock(currentBlock)

      if (currentBlock.getText().length === 0) {
        if (configBlock && configBlock.handleEnterWithoutText) {
          configBlock.handleEnterWithoutText(this, currentBlock)
          this.closePopOvers()
          return true
        }
      ...
      if (currentBlock.getText().length > 0) {
        if (configBlock && configBlock.handleEnterWithText) {
          configBlock.handleEnterWithText(this, currentBlock)
          this.closePopOvers()
          return true
        }
  ...
  setDirection (directionType) {
    ...
    return this.updateBlockData(block, { direction: directionType })
  }
  ...
  render () {
    return (
      <div id='content' suppressContentEditableWarning>
